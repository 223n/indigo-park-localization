...
1. 変更の前にIssueを立て、何を変えたいかを書きます。小さな修正はIssue無しでも構いません。訳文の直しや言い回しの提案は「翻訳の訂正と提案」のフォームを使います
1. `develop`から作業用のブランチを切ります（`feature/変更の名前`）
1. 変更を加え、`npm run lint`が通ることを確かめます。訳文や原文の一覧を変えたときは、`python tools/check_translation.py`も通ることを確かめます
...
| `hotfix/*` | リリース済みの内容の緊急の修正です。`main`から切り、そのブランチで`package.json`の版も上げます。`main`にマージするとタグとドラフトのReleaseが作られ、`develop`にも戻されます。公開まで進めるときは、マージの前に「自動公開」ラベルを付けます |
| `merge/*` | `main`の内容を`develop`へ戻すためのブランチです。「リリースを公開する」ワークフローが作ります。手で取り込むときも使います |
